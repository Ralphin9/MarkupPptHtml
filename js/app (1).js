...
(function () {
  ...
  function setupScriptToVideo() {
    ...
    const elAiMode = modal.querySelector('#s2v-ai-mode');
    const elOpenAiBase = modal.querySelector('#s2v-openai-base');
    const elOpenAiModel = modal.querySelector('#s2v-openai-model');
    const elOpenAiKey = modal.querySelector('#s2v-openai-key');
    const elAiRewrite = modal.querySelector('#s2v-ai-rewrite');
    ...
    const elServerStatus = modal.querySelector('#s2v-server-status');

    function setServerStatus(state) {
      if (!elServerStatus) return;
      const map = {
        checking: ['⟳ checking…',  '#2a2a2a', '#aaa',    'inline'],
        up:       ['● running',    '#1a3a1a', '#4caf50', 'inline'],
        down:     ['✕ not running','#3a1a1a', '#f55',    'inline'],
      };
      const [text, bg, color] = map[state] || [];
      if (!text) { elServerStatus.style.display = 'none'; return; }
      elServerStatus.textContent = text;
      elServerStatus.style.background = bg;
      elServerStatus.style.color = color;
      elServerStatus.style.display = 'inline';
    }
    ...
    function applyServerMode() {
      ...
      if (m === 'hf') {
        elServer.style.display = 'none'; elServer.value = '';
        setServerStatus(null);
      } else if (m === 'local') {
        elServer.style.display = 'block'; elServer.value = 'http://localhost:8001';
        pingServer('http://localhost:8001');
      } else {
        elServer.style.display = 'block';
        if (!elServer.value) elServer.value = 'http://';
        elServer.focus();
        setServerStatus(null);
      }
    }
    ...
    elServer.addEventListener('change', () => {
      if (elServerMode.value !== 'hf' && elServer.value.startsWith('http')) pingServer(elServer.value);
    });

    function applyAiMode() {
      modal.dataset.ai = elAiMode?.value || 'off';
    }
    if (elAiMode) elAiMode.addEventListener('change', applyAiMode);
    ...
    btnGo.addEventListener('click', async () => {
      ...
      try {
        const result = await window.ScriptToVideo.run({
          ...
          ai: elAiMode?.value === 'openai' ? {
            provider: 'openai',
            apiBase: (elOpenAiBase?.value || '').trim(),
            apiKey: (elOpenAiKey?.value || '').trim(),
            model: (elOpenAiModel?.value || '').trim(),
            rewrite: !!elAiRewrite?.checked,
          } : null,
          onProgress: log,
        });
